...
local BEFORE_START = 7                 -- За сколько дней до начала праздника будут начинаться скидки, если время проведения б
 
local WEEK_DISCOUNT_ENABLE = true       -- Будут ли действовать скидки по выходным
local WEEK_DISCOUNT = 10                -- Сколько будет действовать процентов скидка на товары
local IGNORE_WEEKEND = true             -- Будут ли игнорироваться скидки по выходным, во время проведения праздничных

local HOLIDAY_DISCOUNT = 20             -- Сколько будет действовать процентов скидка на товары
...
if WEEK_TBL[os.date("%A", os.time())] and WEEK_DISCOUNT_ENABLE then
    ...
    if SERVER then
        disountNotification("В автодонате(F1) действуют скидки (" .. WEEK_DISCOUNT .. "%) на все товары.", "Скидки продлятся до: " .. os.date("%d", os.time() + (WEEK_TBL[os.date("%A", os.time())] * 86400)) .. os.date(".%m", os.time()))
    end
else
    if HOLIDAY and HOLIDAY_DATE_STAMP then
        for k,v in ipairs(IGS.GetItems()) do
        ...
        if SERVER then
            disountNotification("В автодонате(F1) действуют скидки " .. tostring(HOLIDAY_DISCOUNT) .. [[% на все товары. в честь праздника "]] .. HOLIDAY .. [["]], [[ Скидки продлятся до ]] .. os.date("%d.%m.%y", HOLIDAY_DATE_STAMP + (HOLIDAY_DURATION * 86400)))
        end
    end
end
