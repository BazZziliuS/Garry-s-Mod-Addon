...
hook.Add("Holiday.Create", "CUSTOM.HOLIDAYS", function() 
    ...
    AddCustomHoliday("День рождение сервера", os.date("%Y-04-05"))
    AddCustomHoliday("День рождение Гл.Администратора", os.date("%Y-07-14"))
end)
...
if WEEK_TBL[os.date("%A", os.time())] and WEEK_DISCOUNT_ENABLE then
    ...
    if SERVER then
        disountNotification("В донат меню(F1) действуют скидки (" .. WEEK_DISCOUNT .. "%) на все товары.", "Скидки продлятся до: " .. os.date("%d", os.time() + (WEEK_TBL[os.date("%A", os.time())] * 86400)) .. os.date(".%m", os.time()))
    end
else
    if HOLIDAY and HOLIDAY_DATE_STAMP then
        for k,v in ipairs(IGS.GetItems()) do
        ...
        if SERVER then
            disountNotification("В донат меню(F1) действуют скидки " .. tostring(HOLIDAY_DISCOUNT) .. [[% на все товары. в честь праздника "]] .. HOLIDAY .. [["]], [[ Скидки продлятся до ]] .. os.date("%d.%m.%y", HOLIDAY_DATE_STAMP + (HOLIDAY_DURATION * 86400)))
        end
    end
end
