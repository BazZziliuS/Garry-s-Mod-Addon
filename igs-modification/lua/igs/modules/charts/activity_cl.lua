...
local function addTab(activity,sidebar)
	...
	local categs = uigs.Create("igs_panels_layout_list", bg) -- center panel
	--categs:DisableAlignment(true)
	categs:SetWide(activity:GetWide())
	categs:Dock(FILL)

	for _,CHART in pairs(IGS.CHARTS.GetMap()) do
		categs:Add( uigs.Create("igs_table", function(pnl)
			pnl:SetSize(280,215)
	...
	activity:AddTab("Чарты",bg,"materials/icons/fa32/bar-chart.png")
end
