if SERVER then
	...
	net.Receive( "ScreengrabRequest", function( _, ply )
		...
		for k, v in next, player.GetAll() do
			if v.isgrabbing and v ~= ply then
				ply:rtxappend( sg, red, "Ошибка: " .. v:Name() .. "захват экрана  " .. v.sg .. ". Чтобы уменьшить нагрузку на сервер, можно выполнить захват только одного экрана одновременно." )
				return
			end
		end
		if ply.isgrabbing then
			ply:rtxappend( sg.red, "Ошибка: вы уже захватываете экран кого-то" )
			return
		end
		if not IsValid( ent ) then
			ply:rtxappend( sg.red, "Ошибка: недопустимая цель" )
			return
		end
		if ply:CanScreengrab() then
			...
			ply:rtxappend( sg.red, "Ошибка: недостаточно прав" )
		end
	end )
	...
	net.Receive( "ScreengrabSendPart", function( _, ply )
		...
		if not ply.data then
			ply.data = {}
			ply.data[ 1 ] = data
			sendto:rtxappend( sg.blue, "Полученная 1-я часть" )
		else
			local num = table.getn( ply.data ) + 1
			ply.data[ num ] = data
			sendto:rtxappend( sg.blue, "Полученный " .. num .. STNDRD( num ) .. " часть" )
		end
		if table.getn( ply.data ) == ply.parts then
			ply.IsSending = nil
			sendto:rtxappend( sg.green, "Подготовка к отправке данных [" .. ply.parts .. " часть]" )
			local i = 1
			timer.Create( "SendDataBack", 0.1, ply.parts, function()
				...
				net.Send( sendto )
				sendto:rtxappend( sg.yellow, "Отправлена " .. i .. STNDRD( i ) .. " часть" )
				i = i + 1
			end )
		end
	end )
	...
	hook.Add( "PlayerDisconnected", "ScreengrabInterrupt", function( ply )
		if ply.IsSending then
			...
			ply.isgrabbing = nil
			_ply.isgrabbing = nil
			_ply:rtxappend( sg.red, "Цель отключена до завершения отправки данных" )
...
if CLIENT then
	...
	local render_Capture=render_Capture or render.Capture -- нарушение визуализации.Захват является распространенным способом предотвращения захвата экрана
	local util_Base64Encode=util_Base64Encode or util.Base64Encode -- ломаю util.Base64Encode является распространенным способом предотвращения screengrabbing
	local util_Compress=util_Compress or util.Compress -- ломаю util.Сжатие-еще один возможный способ предотвратить захват экрана
	hook.Add("PostRender", "Screengrab", function()
		if (!shouldScreengrab) then return end
		shouldScreengrab = false
		cl_rtxappend2( sg.green, "Инициализация", _ply )		
		local function capture( q )
			...
			cl_rtxappend2( color_white, "Захваченный " .. len .. " байты", _ply )
			...
			net.SendToServer()
			cl_rtxappend2( Color( 0, 255, 0 ), "Подготовка к отправке данных", _ply )
			local i = 1
			timer.Create( "ScreengrabSendParts", 0.1, amt, function()
				...
				net.SendToServer()
				cl_rtxappend2( Color( 255, 255, 0 ), "Отправленный " .. i .. STNDRD( i ) .. " часть", _ply )
				net.Start( "Progress" )
					net.WriteEntity( _ply )
	...
	local function DisplayData( str, name )
		...
		if not name then
			main:SetTitle( "Экрана  " .. LocalPlayer().gfname .. " (" .. string.len( str ) .. " байт, взял " .. elapsedtime .. " секунды)" )
		else
	...
	net.Receive( "ScreengrabInterrupted", function()
		cl_rtxappend( sg.red, "Соединение с целью прервано" )
	...
	net.Receive( "SendPartBack", function()
		...
		if table.getn( LocalPlayer().sgtable ) == LocalPlayer().parts then
			...
			cl_rtxappend( sg.green, "Законченный" )
	...
	function OpenSGMenu()
		...
		main.Paint = function()
			...
			surface.DrawText( "Screengrab Меню " )
		end
		...
		files.OnRowRightClick = function( main, line )
			local menu = DermaMenu()
				...
				menu:AddOption( "Посмотреть Скриншот", function()
		...
		save.DoClick = function()
			...
			cl_rtxappend( Color( 255, 100, 0 ), "Сохранение в файл: " .. name .. " (" .. string.len( text ) .. " байты)" )
			file.Write( "screengrabs/" .. name, text )
			timer.Simple( 1, function()
				if file.Exists( "screengrabs/" .. name, "DATA" ) then
					cl_rtxappend( Color( 255, 100, 0 ), "Снимок экрана сохранен!" )
				else
					cl_rtxappend( Color( 255, 0, 0 ), "Ошибка: снимок экрана не сохранен!" )
				end
			end )
		end
