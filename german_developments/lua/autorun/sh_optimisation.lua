 ...
 if CLIENT then
	 hook.Add("PlayerFinishedLoading","CSS_Check.PlayerIsLoaded",function()
		if IsMounted(240)
			...
			local msg1 = '[ОШИБКА] Похоже, у тебя нет контента Counter-Strike: Source, скачай ее в Steam или скачать контент тут: https://votguide.ru/gajdy/ustanavlivaem-css-kontent-dlya-garrys-mod/'
			chat.AddText(Color(200, 0, 0), msg1)
		end  
 	    
 	    local msg2 = '[ОШИБКА] У вас заблокирован скачиваемый игровой контент. Во избежание ошибок, пропишите в консоль: "cl_downloadfilter all; cl_allowdownload 1;'
		local hasservercontent = file.Exists('banner.png', 'DATA')
        if not hasservercontent then
            chat.AddText(Color(200, 0, 0), msg2)
        end
