--[[
Если не работает - пиши в консоль bind alt +walk

Если на альт не нравится - пиши в консоль bind КНОПКА +freelook

Серверные конвары для настройки:
- altlook 1 (вкл/выкл)
- altlook_limit_horizontal 140 (на сколько градусов можно повернуть голову горизонатально)
- altlook_limit_vertical 65 (вертикально)
- altlook_block_fire 1 (блокировать кнопку стрельбы если нажат alt. Полезно, потому что в камере отдача не работает)
- altlook_block_ads 1 (отключить осмотр при прицеливании)
- altlook_smoothness_mult 1 (множитель скорости)
]]--
...
hook.Add("CalcView", "AltlookView", function(ply, pos, ang, fov, znear, zfar)
    ...
    if not holdingbind(ply) and CoolAng.p < 0.05 or isinsights(ply) and CoolAng.p < 0.05 or not system.HasFocus() or ply:ShouldDrawLocalPlayer() then InitialAng, LookX, LookY = ang, 0, 0 CoolAng = Angle() return end
