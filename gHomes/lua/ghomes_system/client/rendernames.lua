...
if ghomes.enabledhousenames then
	hook.Add("PostDrawOpaqueRenderables", "housesnamerender3d", function()
		...
		for k, v in ipairs(ghomes.list) do
			...
			local color = Color(255, 255, 255, math.Min(210, math.Remap(dist, textFadeOutDist, textMaxDist, 120, 0)))
			...
			draw.RoundedBox(10, 0, 0, width + 50, height-46, colordark)
...
hook.Add("UpdatedHouseList", "CreatePanel", function(id)
	...
	for panelID in pairs(house.panelpos) do
		...
		curDerma.Paint = function(pnl, w, h)
			...
			draw.RoundedBoxEx(12, 0, 0, w, 48, Color(40, 40, 40, curDerma.alpha), true, true, false, false)
			draw.RoundedBoxEx(12, 0, 48, w, 332, Color(60, 60, 60, curDerma.alpha), false, false, true, true)
			local status = ""
			...
			if not house.isrented then
				local t_color = Color(255, 255, 255, curDerma.alpha)
				local m_color = Color(40, 40, 40, curDerma.alpha)
				...
				if not house.ispermarented then
					draw.RoundedBoxEx(12, 0, h - 25, w, 25, Color(40, 40, 40, curDerma.alpha - 150), false, false, true, true)
					...
					if time >= 0 then
						draw.SimpleText(Format(ghomes.leaseexpiresin, text), "DrawHouseNameSmall5", w / 2, h - 22, color_white, TEXT_ALIGN_CENTER)
					else
						draw.SimpleText(Format(ghomes.leasehasexpired, text), "DrawHouseNameSmall5", w / 2, h - 22, color_white, TEXT_ALIGN_CENTER)
					end
				end
			end
		end
		...
		if not house.isrented then
			...
			buttonBuy.Paint = function(pnl, w, h)
				...
				if (pnl.Hovered) then
					m_color = Color(80, 80, 80, curDerma.alpha)
				end
				...
				draw.SimpleText(text, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
			end
		else
			...
			buttonRing.Paint = function(pnl, w, h)
				...
				surface.DrawTexturedRect(0, 0, 48, 48)
				draw.SimpleText(ghomes.ringthebell, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
			end
			...
			if isOwner or isFriend then
				...
				buttonReadMails.Paint = function(pnl, w, h)
					...
					surface.DrawTexturedRect(0, 0, 48, 48)
					draw.SimpleText(ghomes.readmail, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
					draw.SimpleText(tostring(LocalPlayer().nmails), "DrawHouseNameSmall24Bold", 23, 10.5, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_CENTER)
				end
			else
				...
				buttonLeaveAMessage.Paint = function(pnl, w, h)
					...
					surface.SetMaterial(letter)
					surface.DrawTexturedRect(0, 0, 48, 48)
					draw.SimpleText(ghomes.leaveamessage, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
				end
			end
			...
			if isOwner then
				if not house.ispermarented then
					...
					buttonReNew.Paint = function(pnl, w, h)
						...
						surface.DrawTexturedRect(0, 0, 48, 48)
						draw.SimpleText(ghomes.renewthelease, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
					end
				end
				...
				buttonCoOwners.Paint = function(pnl, w, h)
					...
					draw.SimpleText(ghomes.coowners, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
				end
				...
				buttonConfirmSell.Paint = function(pnl, w, h)
					...
					surface.DrawTexturedRect(0, 0, 48, 48)
					draw.SimpleText(selltext, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
				end
				if ghomes.dlcs.dlc1 then
					...
					if not house.ispermarented then
						...
						buttonSettings.Paint = function(pnl, w, h)
							...
							surface.SetMaterial(gear)
							surface.DrawTexturedRect(2, 0, 48, 48)
							draw.SimpleText(ghomes.settings, "DrawHouseNameSmall32", 55, 0, Color(255, 255, 255, curDerma.alpha), TEXT_ALIGN_LEFT)
						end
