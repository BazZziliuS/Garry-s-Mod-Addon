...
local color2 = Color(220, 220, 220, 255)
local color_grey = Color(200, 200, 200)
local color_white =  Color(40, 40, 40, 255)
...
function ghomes.confirmcreation(housedata, selectedhouse)
	...
	local n = Color(60, 60, 60)
	local y = Color(40, 40, 40)
...
function ghomes.sellermenu()
	...
	local function preview()
		...
		local t_color = Color(220, 220, 220, 255)
		local m_color = Color(80, 80, 80, 80)
	...
	local color_black = Color(240, 240, 240)
	local radius = 8
			...
			do
				...
				elseif isOwner or LocalPlayer():CanMasterHouse() then
					...
					if ghomes.dlcs.dlc1 then
						...
						buttonConfig.Paint = function(pnl, w, h)
							...
							draw.DrawText("Изменить", "GuiSentData1", w / 2, 60, color_green, TEXT_ALIGN_CENTER)
						end
				...
				if LocalPlayer():CanMasterHouse() then
					...
					buttonAdmin.Paint = function(pnl, w, h)
						...
						surface.SetMaterial(gear)
						surface.DrawTexturedRect(w / 2 - 32, 3, 64, 64)
						draw.DrawText("Изменить", "GuiSentData1", w / 2, 60, color_black, TEXT_ALIGN_CENTER)
					end
					...
					buttonAdmin.Paint = function(pnl, w, h)
						...
						surface.DrawTexturedRect(w / 2 - 32, 3, 64, 64)
						draw.DrawText("Удалить", "GuiSentData1", w / 2, 60, color_black, TEXT_ALIGN_CENTER)
					end
...
function ghomes.edithouse(houseid, panelptr)
	...
	local color42 = Color(80, 80, 80)

	DermaPanel.Paint = function(pnl, w, h)
		draw.RoundedBox(8, 0, 0, w, h, color2)
		draw.RoundedBox(8, 1, 1, w - 2, h - 2, color42)
		draw.SimpleText("Название", "GuiSentData1", 25, 20, color_grey, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
		draw.SimpleText("Режим аренды/покупки", "GuiSentData1", 25, 70, color_grey, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
		draw.SimpleText("Цена за сутки (Аренда)", "GuiSentData1", 25, 120, color_grey, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
		draw.SimpleText("Цена покупки (Постоянная)", "GuiSentData1", 25, 170, color_grey, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	end
	...
	local TextEntryTitle = vgui.Create("DTextEntry", DermaPanel)
	TextEntryTitle:SetPos(130, 20)
	TextEntryTitle:SetSize(130, 30)
	TextEntryTitle:SetDrawLanguageID(false)
	TextEntryTitle:SetText(ghomes.list[houseid].name)
	local DComboBoxRentalmode = vgui.Create("DComboBox", DermaPanel)
	DComboBoxRentalmode:SetPos(230, 70)
	DComboBoxRentalmode:SetSize(150, 30)
	DComboBoxRentalmode:SetSortItems(false)
